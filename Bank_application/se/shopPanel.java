		...
		public void actionPerformed(ActionEvent e){
			if(e.getSource() == shopCb){
				...
				case 1: //도스마스
					...
					imagePanel.add(imageLabel);		// 도스마스 페이지로 오면 도스마스 이미지로 바귄다.
								...
								if(ok == JOptionPane.OK_OPTION){ //확인 버튼 눌렀을 시
									...
									if(user == null){ //고객이 아니다 -> 접속 실패 알림
										...
										if((SE.smartLoginState == true) && (SmartPhonePanel.userAccountNumber.getText().equals(user))){ 
											if(SE.banking(user, shopCb.getSelectedItem() + "", result, '-', true)){ //거래 진행
												payClear(); //내역 초기화
											}
										}
										else{
											if(SE.banking(user, shopCb.getSelectedItem() + "", result, '-', false)){ //거래 진행
												payClear(); //내역 초기화
											}
										}
									}
